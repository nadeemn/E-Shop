...
function BagPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const response = await fetch(
                    "http://127.0.0.1:5000/api/getFilteredBags",
                    {
                        method: "POST",
                        headers: {
                        ...
                        body: JSON.stringify({
                            bag_checkbox_options: selectedFilterItems,
                        }),
                    }
                );
                const filteredItem = await response.json();
                setBagData(filteredItem);
                setTimeout(() => {
                    setIsLoading(false);
                }, 1000);
            } catch (error) {
        ...
        const fetchAllData = async () => {
            try {
                const response = await fetch("http://127.0.0.1:5000/api/getBagPrducts");
    ...
    if (bagData.length > 0) {
        var actualData = bagData.map((item) => (
            <Box
                key={item._id}
