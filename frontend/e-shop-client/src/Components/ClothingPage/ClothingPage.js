...
function ClothingPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const response = await fetch(
                    "http://127.0.0.1:5000/api/getFilteredClothes",
                    {
                        method: "POST",
                        headers: {
                        ...
                        body: JSON.stringify({
                            cloth_checkbox_options: selectedFilterItems,
                        }),
                    }
                );
                const filteredItem = await response.json();
                setClothData(filteredItem);
                setTimeout(() => {
                    setIsLoading(false);
                }, 1000);
            } catch (error) {
        ...
        const fetchAllData = async () => {
            try {
                const response = await fetch(
                    "http://127.0.0.1:5000/api/getClothPrducts"
                );
    ...
    var GenderCheckboxes = ClothingGender.map((item, i) => (
        <FormControlLabel
            key={i}
            control={
    ...
    var CategoryCheckboxes = ClothingCategories.map((item, i) => (
        <FormControlLabel
            key={i}
            control={
                <Checkbox
    ...
    if (clothData.length > 0) {
        var actualData = clothData.map((item) => (
            <Box
                key={item._id}
