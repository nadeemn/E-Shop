...
export const MyInbox = () => {
    ...
    useEffect(() => {
        fetch("http://127.0.0.1:5000/api/retrieveChat", {
            headers: {
    ...
    const handleRetrieveChat = async (senderid) => {
        setCurrentCustomer(senderid)
        const resposne = await fetch(
            "http://127.0.0.1:5000/api/retrieveChatMessage",
            {
                method: "POST",
                headers: {
                    Authorization: "Bearer " + sessionStorage.getItem("token"),
    ...
    const handleSendReply = async () => {
        const response = await fetch(
            "http://127.0.0.1:5000/api/sellerReply",
            {
                method: "POST",
                headers: {
                    Authorization: "Bearer " + sessionStorage.getItem("token"),
        ...
        if (response.status === 200) {
                alert("message sent");
                handleRetrieveChat(currentCustomer);
                setReply("");
        }
    };
        ...
        var conversation = textChat.map((item, i) => (
            <ListItem key={i}>
                <Grid container>
                    <Grid item xs={12}>
                        <ListItemText
                            align={item._receiver_id === currentCustomer ? "right" : "left"}
