...
function MobilePage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const response = await fetch(
                    "http://127.0.0.1:5000/api/getFilteredElectronics",
                    {
                        method: "POST",
                        headers: {
                        ...
                        body: JSON.stringify({
                            electronic_checkbox_options: selectedFilterItems,
                        }),
                    }
                );
                const filteredItem = await response.json();
                setElectronicData(filteredItem);
                setTimeout(() => {
                    setIsLoading(false);
                }, 1000);
            } catch (error) {
        ...
        const fetchAllData = async () => {
            try {
                const response = await fetch(
                    "http://127.0.0.1:5000/api/getElectronicPrducts"
                );
    ...
    var BrandCheckboxes = Brands.map((item, i) => (
        <FormControlLabel
            key={i}
            control={
                <Checkbox
    ...
    if (electronicData.length > 0) {
        var actualData = electronicData.map((item) => (
            <Box
                key={item._id}
