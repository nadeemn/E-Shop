...
function EssentialPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const response = await fetch(
                    "http://127.0.0.1:5000/api/getFilteredEssentials",
                    {
                        method: "POST",
                        headers: {
                        ...
                        body: JSON.stringify({
                            essential_checkbox_options: selectedFilterItems,
                        }),
                    }
                );
                const filteredItem = await response.json();
                setEssentialData(filteredItem);
                setTimeout(() => {
                    setIsLoading(false);
                }, 1000);
            } catch (error) {
        ...
        const fetchAllData = async () => {
            try {
                const response = await fetch(
                    "http://127.0.0.1:5000/api/getEssentialPrducts"
                );
        ...
        if (selectedFilterItems.key1.length > 0) {
            fetchData();
        } else if (selectedFilterItems.key1.length === 0) {
            fetchAllData();
        }
    }, [selectedFilterItems]);
    ...
    if (essentialData.length > 0) {
        var actualData = essentialData.map((item) => (
            <Box
                key={item._id}
